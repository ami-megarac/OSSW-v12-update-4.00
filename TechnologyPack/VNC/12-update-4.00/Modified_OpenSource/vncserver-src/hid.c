...
u8 CalculateModulo100(u8 *Buff, int BuffLen) {
	...
	for (i = 0; i < BuffLen; i++)
		ChkSum += Buff[i];
	return (~ChkSum + 1); // 2's compliment calculation changed from (-ChkSum) to (~ChkSum + 1) for fixing Fortify [Type Mismatch: Signed to Unsigned]
}
...
int mousePkt(int buttonMask, int x, int y, int maxx, int maxy) {
	...
	mouseRep.Event = (uint8) ((buttonMask == 4) ? 2 : buttonMask);
	mouseRep.X = (uint16) X;
	mouseRep.Y = (uint16) Y;
	mouseRep.Scroll = 0;
