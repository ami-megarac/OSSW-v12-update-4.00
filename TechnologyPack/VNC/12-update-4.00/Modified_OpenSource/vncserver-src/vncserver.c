...
static void clientgone(rfbClientPtr cl) {
	...
	{	// vnc_max_session, sessionInfo values are initialized upon receiving RECV_VNC_CONF command in pipe
		memset((char *)sessionInfo, 0, (sizeof(VNCSessionInfo) * vnc_max_session)); /* Fortify [Buffer Overflow]:: False Positive */
...
int setupFb(struct fb_info_t* fb_info) {
	...
	emptyScreen= (char*)malloc(maxx*maxy*bpp); /* Fortify [Integer Overflow]:: False Positive */
...
void adoptResolutionChange(rfbClientPtr cl) {
	...
	iterator = rfbGetClientIterator(cl->screen); /* Fortify [Memory Leak]:: False Positive */
...
void showBlankScreen(char *message,int xPos,int yPos) {
	...
	if((emptyScreen != NULL) && (emptyScreenSize > 0))	// Fix for Fortify [Missing Check against Null], Values are initialized inside setupFb()
		memset(emptyScreen, 0, emptyScreenSize); /* Fortify [Buffer Overflow]:: False Positive */
...
void cleanUpInvalidSession(int sessionIndex, int reason)
{
	...
	sessionInfo[sessionIndex].status = UNUSED; /* Fortify [Buffer Overflow]:: False Positive */
...
{	// vnc_max_session, sessionInfo values are initialized upon receiving RECV_VNC_CONF command in pipe
	if (NULL != (sessionInfo = (VNCSessionInfo *) malloc ((sizeof(VNCSessionInfo) * vnc_max_session))) ) // Fix for Fortify [Missing Check against Null]
		memset((char *)sessionInfo, 0, (sizeof(VNCSessionInfo) * vnc_max_session)); /* Fortify [Intger Overflow]:: False Positive */

	return sessionInfo;
}
pthread_mutex_t  *getVncSessionGuard()
{	// vnc_max_session value is initialized upon receiving RECV_VNC_CONF command in pipe
	if (NULL == (vnc_session_guard = (pthread_mutex_t *)calloc(vnc_max_session, sizeof(pthread_mutex_t))) ) /* Fortify [Intger Overflow]:: False Positive */
...
int openVncPipe(char *pipename) {
	...
	if (pipename != NULL) {
		...
		fd = open(pipename, O_RDWR|O_NONBLOCK); /* Fortify [Race Condition: File System Access]:: False Positive */
		if (fd < 0) {
		...
		if (read(vncpipe, &cmd, sizeof(cmd)) > 0) {
			...
			switch (cmd.cmd)
			{
				...
				case RECV_VNC_CONF:
					if(cmd.datalen > 0)
					{
						if(read(vncpipe, &vncConf, cmd.datalen) > 0) /* Fortify [Buffer Overflow]:: False Positive */
						{
				...
				case DISCONNECT_VNC_SESSION:
					...
					if(read(vncpipe, &vnc_session, sizeof(vnc_clean_session_t)) > 0) /* Fortify [Buffer Overflow]:: False Positive */
					{
						sessionInfo[ vnc_session.session_index ].status = vnc_session.reason; /* Fortify [Buffer Overflow]:: False Positive */
					}
					else
					{
...
int WriteCmdInAMIVncPipe(int cmd, int status){
	    ...
	    if(AmiVNCPipe != -1)
	    {
	        comm_info.cmd = (unsigned short) cmd;
	        comm_info.datalen = sizeof(cmd_info_t);
	        comm_info.status = (unsigned short) status;
