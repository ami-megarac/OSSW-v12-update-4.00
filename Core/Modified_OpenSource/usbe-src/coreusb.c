	...
	for (IfaceNum=0;IfaceNum<bNumInterfaces;IfaceNum++)
	{
		...
		for (EpNum = 1; ((EpNum <= TotalEndPoints) && bNumEndPoints); EpNum++)
		{
			for (Direction = DIR_OUT; Direction <= DIR_IN; Direction++)
			{
				...
				{
					bNumEndPoints -= 1;
					EpDir = (EndDesc->bEndpointAddress & IN)?DIR_IN:DIR_OUT; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */
...
/*	EpInfo->Enabled 	  	= 1; */
	...
	else
	{
		...
		EpInfo->Dir				= (EndDesc->bEndpointAddress & 0x80)?DIR_IN: DIR_OUT; /* Fortify [Type Mismatch: Signed to Unsigned]:: False Positive */
	}
