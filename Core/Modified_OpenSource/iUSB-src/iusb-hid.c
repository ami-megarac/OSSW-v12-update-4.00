...
KeybdPrepareData(IUSB_HID_PACKET *UserModePkt, uint8 *KeybdData)
{
	...
	memcpy (KeybdData, &(HidPkt->Data),  8); /* Fortify [Out-of-Bounds Read]:: False Positive */
	return 0;
}
...
MousePrepareData(IUSB_HID_PACKET *UserModePkt, uint8 MouseMode, uint8 *MouseData)
{
	...
	if (MouseMode == RELATIVE_MOUSE_MODE)
	{
		...
		memcpy (MouseData, &(HidPkt->Data), REL_EFFECTIVE_MOUSE_DATA_SIZE); /* Fortify [Out-of-Bounds Read]:: False Positive */
		return 0;
	}
	else if (MouseMode == ABSOLUTE_MOUSE_MODE)
	{
		...
		memcpy (MouseData, &(HidPkt->Data), ABS_EFFECTIVE_MOUSE_DATA_SIZE); /* Fortify [Out-of-Bounds Read]:: False Positive */
		return 0;
    }
	else 
